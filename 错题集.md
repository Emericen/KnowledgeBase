...
**Solution (No Duplicated inversions)**

```java
public static int solve(int[] array) {
    // TX = O(N^2)
    // SC = O(N)
    int[] smallRightCount = new int[array.length];
    Set<Integer> visited = new HashSet<>();
    for (int i = 0; i < array.length; i++) {
        visited.clear();
        for (int j = i + 1; j < array.length; j++) {
            if (visited.contains(array[j])) {
                continue;
            }
            visited.add(array[j]);
            if (array[i] > array[j]) {
    ...
    for (int i = 0; i < array.length; i++) {
        ...
        visited.add(array[i]);
        for (int j = i + 1; j < array.length; j++) {
            if (visited2.contains(array[j]) || array[j] >= array[i]) {
                continue;
            }
            visited2.add(array[j]);
            result += smallRightCount[j];
        }
        visited2.clear();
    }
    return result;
